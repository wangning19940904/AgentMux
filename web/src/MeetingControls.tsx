...
// Incoming invitations remain global even though manual joining now belongs
// to the Meetings page. This keeps time-sensitive calls visible on every tab.
export function MeetingInvitationOverlay() {
  const { t } = useI18n();
  ...
  return createPortal(
    <>
      {toast && <MeetingToast message={toast} />}
      {pendingInvitation && (
        <div className="meeting-dialog-layer" role="presentation">
          ...
          >
            ...
            {pendingInvitation.last_error && <div className="session-notice error">{pendingInvitation.last_error}</div>}
...
export function MeetingJoinControl() {
  const { t } = useI18n();
  const { overview, refresh } = useMeetings();
  ...
  useEffect(() => {
    if (overview.invitations.length > 0) setJoinOpen(false);
  ...
  async function submitJoin(event: FormEvent) {
    ...
    try {
      ...
      setJoinOpen(false);
      setMeetingNumber("");
      setToast(result.greeting_warning
        ? t("meeting.joinedGreetingWarning", { error: result.greeting_warning })
        : t("meeting.joinedAndGreeted"));
      await refresh();
    } catch (error) {
      setDialogError(error instanceof Error ? error.message : String(error));
    } finally {
  ...
  return (
    <>
      <button className="action meeting-launch-button" type="button" onClick={openJoinDialog}>
        <Video size={16} />
            ...
            <div className="meeting-dialog-layer" role="presentation">
              ...
              >
                ...
                <form className="meeting-join-form" onSubmit={submitJoin}>
                  ...
                  {connectedChannels.length === 0 && <div className="session-notice error">{t("meeting.noConnectedChannels")}</div>}
                  ...
                  <div className="meeting-dialog-actions">
                    ...
                    <button
                      ...
                      type="submit"
                    >
                      {busy ? <LoaderCircle className="spin" size={16} /> : <Video size={16} />}
                      {t("meeting.joinNow")}
                    </button>
                  </div>
                </form>
...
function MeetingToast({ message }: { message: string }) {
  return (
