...
发布维护说明见 [RELEASING.md](RELEASING.md)。运行时使用 PostgreSQL 16+；
macOS/Linux 默认本地数据库会在安装脚本执行时或首次启动时自动检测、补装并初始化。
配置外部数据库时直接使用该连接。权限要求与失败重试见 [安装说明](INSTALL.md#set-up-postgresql)。
...
在 **Agent → 编辑 → 会话模式** 设置飞书/Lark 的默认模式，适用于该 Agent 绑定的所有渠道。聊天内 `/mode` 打开选择卡或 `/mode <模式>` 可单独覆盖；私聊覆盖只影响本人，群模式由群主或群管理员修改。历史会话不合并、不删除，模式变更只影响之后的消息，已有任务和控制卡保留原关联。
...
| 普通群 | `chat-topic`（默认） | 直接在群里发消息：机器人在群里单独回复，共享上下文；在话题里发消息：在话题内回复，各话题使用独立上下文 |
| 普通群 | `new-topic` | 每条直接发到群里的消息创建独立话题，机器人在话题内回复；同一话题内共享上下文 |
| 普通群 | `chat` | 群内所有消息共用上下文，机器人统一在群聊主界面回复，包括从话题里发起的消息 |
| 话题群 | 固定话题模式 | 每个话题独立 |

Agent 字段为 `private_chat_mode` 与 `group_chat_mode`，新建 Agent 默认分别为 `chat` 和 `chat-topic`。优先级为聊天内 `/mode` 覆盖 → Agent 默认 → 旧渠道设置 → 系统默认。旧 Agent 未设置这些字段时，界面显示“保留各渠道现有设置”，继续读取原渠道配置；选定 Agent 模式后覆盖所有绑定渠道的旧值。聊天覆盖设置持久化保存，飞书话题群始终按话题隔离。专属会话群内可直接发送消息，无需逐条 @机器人。建群使用稳定 UUID 重试；明确失败时退回私聊独立话题，结果不确定时可使用提示中的 `/retry-group <消息ID>` 重试同一请求。
