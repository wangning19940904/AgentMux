// Derived from github.com/larksuite/oapi-sdk-go/v3/ws v3.9.9 (MIT).
// Local lifecycle fixes keep dynamic event subscriptions cancellable.

// Package ws provides a cancellable Lark WebSocket event client.
package ws
...
var bootstrapHTTPClient = &http.Client{Timeout: 10 * time.Second}
...
func (c *Client) reconnect(ctx context.Context) (err error) {
	...
	if reconnectNonce > 0 {
		...
		if !waitContext(ctx, time.Duration(num)*time.Millisecond) {
			return ctx.Err()
		}
	}
	...
	if reconnectCount >= 0 {
		for i := 0; i < reconnectCount; i++ {
			...
			if !waitContext(ctx, reconnectInterval) {
				return ctx.Err()
			}
		}
...
func (c *Client) pingLoop(ctx context.Context) {
	...
	for {
		...
		if !waitContext(ctx, pingInterval) {
			return
		}
	}
}
...
func (c *Client) shouldReconnect(ctx context.Context) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.autoReconnect && ctx.Err() == nil
}
